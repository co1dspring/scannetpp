...
def save_json(data, file_path, indent=4, ensure_ascii=False):
    ...
    :param file_path: 文件路径（如 "data/output.json"）
    :param indent: 缩进空格数（美化格式，None 表示紧凑存储）
    :param ensure_ascii: 是否转义非 ASCII 字符（False 保留中文等）
    """
...
def read_txt_to_list(file_path, keep_newlines=False, skip_empty=False):
    ...
    参数:
        ...
        keep_newlines (bool): 是否保留行尾换行符（默认False）
        skip_empty (bool): 是否跳过空行（默认False）
...
class scannetpp_dataset:
    def __init__(self, data_dir, output_dir, output_json):
        ...
        self.train_scene_list_file = os.path.join(self.data_dir, 'splits/nvs_sem_train.txt')
        self.train_scene_list = read_txt_to_list(self.train_scene_list_file)
        self.sample_rate = 25 
        self.image_type = 'dslr' # 'dslr'
    ...
    def preprocess(self):
        print(len(self.train_scene_list))
        rasterout_dir = Path(self.data_dir) / self.image_type
        
        for scene_id in tqdm(self.existed_scene):
            images_annotation = []
            ...
            if self.image_type == 'dslr':
                undistort_intrinsic = compute_undistort_intrinsic(intrinsic, img_height, img_width, distort_params[:4])
                undistort_map1, undistort_map2 = cv2.fisheye.initUndistortRectifyMap(
                    intrinsic, distort_params[:4], np.eye(3), undistort_intrinsic, (img_width, img_height), cv2.CV_32FC1
                )
            ...
            for i, image_name in enumerate(tqdm(image_list, desc='image', leave=False)):
                ...
                # 这里由于raster过程太慢，并且占据存储空间太大，所以把raster的过程转移到这里，使用缓存
                pose = torch.Tensor(np.array(poses[i:i+1]))
                camera = get_fisheye_cameras_batch(pose, img_height, img_width, intrinsic, distort_params) #这里取决于image_type
                ...
                # visibility_data的数据生成也太臃肿了，需要改写一下
            ...
            save_json(images_annotation, self.output_dir / scene_id / 'obj_annotation_dslr.json', indent=4, ensure_ascii=False)
...
if __name__ == '__main__':
    data_dir = './'
    output_dir = './scannetpp_sampled'
    output_json = './scannetpp_rawdata.json'
