...
def extract_rgb(scene):
    ...
    if scene.iphone_video_path.exists():
        if not os.path.exists(scene.iphone_rgb_dir/'frame_000000.jpg'):
...
def extract_masks(scene):
    ...
    if scene.iphone_video_mask_path.exists():
        if not os.path.exists(scene.iphone_video_mask_dir/'frame_000000.png'):
...
def extract_depth(scene):
    ...
    if scene.iphone_depth_path.exists():
        if not os.path.exists(scene.iphone_depth_dir  / 'frame_000000.png'):
            try:
...
def main():
    ...
    scene_ids = os.listdir(Path('./data'))

    # get the options to process
    # go through each scene
    for scene_id in tqdm(scene_ids, desc='scene'):
        scene = ScannetppScene_Release(scene_id, data_root=Path('./data'))
